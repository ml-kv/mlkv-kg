...
class MLKVEmbedding:
    ...
    def __init__(self, args, num, dim, device):
        ...
        self.emb = None
        self.state_sum = th.empty((num, ), dtype=th.float32, device=device)
        self.state_step = 0
        self.has_cross_rel = False
        # signal used by asynchronous update
        self.embedding_staleness = args.embedding_staleness
        ...
        import os
        import shutil
        mlkv_dir = '/mnt/nvme1n1/testdb'
        if os.path.exists(mlkv_dir):
            shutil.rmtree(mlkv_dir)
        os.makedirs(mlkv_dir)

        self.mlkv = MLKV(table_size_bytes = 128 * pow(2, 20), log_size_bytes = 768 * pow(2, 20),
            path = mlkv_dir, log_mutable_fraction = 0.8, feat_dim = dim, feat_dtype = th.float32)
        self.mlkv.open_mlkv()
    ...
    def init(self, emb_init):
        ...
        step = 10000000
        for i in range(self.num, 0, -step):
            if i - step > 0:
                feat_ids = th.arange(start=i-step, end=i, dtype=th.int64, device='cpu')
                feat_data = th.empty(step, self.mlkv.feat_dim, dtype=th.float32, device='cpu')
            else:
                feat_ids = th.arange(start=0, end=i, dtype=th.int64, device='cpu')
                feat_data = th.empty(i, self.mlkv.feat_dim, dtype=th.float32, device='cpu')
            INIT.uniform_(feat_data, -emb_init, emb_init)
            self.mlkv.parallel_push_data_to_mlkv(num_thread = 8, id_tensor = feat_ids, data_tensor = feat_data)
    ...
    def __call__(self, idx, gpu_id=-1, trace=True):
        ...
        s = self.mlkv.pull_data_from_mlkv(idx)
        if gpu_id >= 0:
            s = s.cuda(gpu_id)
        ...
        with th.no_grad():
            for idx, data in self.trace:
                ...
                if self.async_q is not None:
                    ...
                    data_tensor = self.mlkv.pull_data_from_mlkv(grad_indices)
                    data_tensor += tmp
                    self.mlkv.push_data_to_mlkv(grad_indices, data_tensor)
        self.trace = []

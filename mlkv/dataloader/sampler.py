...
from .beta_sampler import BetaPartition, SetNegRangeUniformEdgeSample
...
class TrainDataset(object):
    ...
    def __init__(self, g, dataset, args, ranks=64, has_importance=False):
        ...
        if ranks > 1 and args.rel_part:
            ...
            self.beta_ordering = True
            self.edge_parts, self.entity_parts = BetaPartition(dataset.n_entities, triples, args.beta_num_part)
    ...
    def create_sampler(self, batch_size, neg_sample_size=2, neg_chunk_size=None, mode='head', num_workers=32,
        ...
        edge_sampler = EdgeSampler(self.g,
        ...
        if self.beta_ordering:
            SetNegRangeUniformEdgeSample(edge_sampler._sampler,
                                         self.entity_parts[rank % len(self.entity_parts)][0],
                                         self.entity_parts[rank % len(self.entity_parts)][1])
        return edge_sampler

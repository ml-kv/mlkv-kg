...
def train(args, model, train_sampler, valid_samplers=None, rank=0, rel_parts=None, cross_rels=None, barrier=None, client=None):
    ...
    for step in range(0, args.max_step):
        start1 = time.time()
        if args.embedding_staleness:
            if len(lookahead_graphs) == 0:
                for _ in range(args.embedding_staleness):
                    pos_g, neg_g = next(train_sampler)
                    model.entity_emb.mlkv.async_parallel_lookahead_data_from_mlkv(
                        num_thread = 1, id_tensor = pos_g.ndata['id'])
                    lookahead_graphs.append((pos_g, neg_g))
            pos_g, neg_g = lookahead_graphs.pop(0)
        else:
            pos_g, neg_g = next(train_sampler)
        ...
        if args.force_sync_interval > 0 and \
            ...
            if barrier is not None:
                barrier.wait()
        if args.embedding_staleness:
            if (step + 1) % args.embedding_staleness == 0:

...
def async_update(args, emb, queue):
    ...
    while True:
        if queue.qsize() <= emb.embedding_staleness / 2 and emb.is_training.value:
            continue
...
class ExternalEmbedding:
    ...
    def __init__(self, args, num, dim, device):
        ...
        self.is_training = th.multiprocessing.Value('b', 1)
    ...
    def __call__(self, idx, gpu_id=-1, trace=True):
        ...
        if self.async_q is not None:
            while self.async_q.qsize() >= self.embedding_staleness:
                pass

...
from dataloader import ConstructGraph, TrainDataset, EvalDataset, NewBidirectionalOneShotIterator
...
class ArgParser(CommonArgParser):
    def __init__(self):
        ...
        self.add_argument('--beta_ordering', action='store_true',
                          help='Enable BETA ordering for single-process training.')
        self.add_argument('--beta_num_part', type=int, default=4,
                          help='Number of partition in BETA ordering.')
...
def main():
    ...
    if args.num_proc > 1:
        ...
        train_sampler = NewBidirectionalOneShotIterator(train_sampler_head, train_sampler_tail,
                                                        ...
                                                        True, dataset.n_entities,
                                                        args.has_edge_importance)
    elif args.beta_ordering:
        train_samplers = []
        for i in range(args.beta_num_part * args.beta_num_part):
            # for each GPU, allocate num_proc // num_GPU processes
            train_sampler_head = train_data.create_sampler(args.batch_size,
                                                           args.neg_sample_size,
                                                           args.neg_sample_size,
    ...
    if args.num_proc > 1:
        ...
        for i in range(args.beta_num_part * args.beta_num_part):
            ...
            import torch
            part_start_id = (i % args.beta_num_part) * n_entities / args.beta_num_part
            part_end_id = part_start_id + n_entities / args.beta_num_part
            id_tensor = torch.arange(start=part_start_id, end=part_end_id,
                dtype=torch.int64, device='cpu')
            model.entity_emb.mlkv.async_parallel_lookahead_data_from_mlkv(
                num_thread = 2, id_tensor = id_tensor)
            train(args, model, train_samplers[i], valid_samplers, rel_parts=rel_parts)
            model.entity_emb.mlkv.wait_async_threads_in_mlkv()
            print('part {} takes {:.3f} seconds'.format(i, time.time() - part_start_time))
    else:
        if args.dataset == "wikikg90M":
            valid_samplers = [valid_sampler_tail] if args.valid else None
        else:
